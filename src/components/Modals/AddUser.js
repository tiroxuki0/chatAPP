...
export default function ModalAddUser() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (uidSelected) {
      modalAddUser(false)
      await updateMemberInRoom("rooms", [...uidSelected, ...roomSelected.members], roomSelected.id)
      const rooms = await getAllDocs("rooms")
      const roomSelectedUpdate = await rooms.filter((room) => room.id == roomSelected.id)
  ...
  return (
    <div>
      <Backdrop sx={{ color: "#fff", zIndex: (theme) => theme.zIndex.drawer + 1 }} open={addUser} onClick={onClickBackdrop} className="backdrop">
        <ModalAddWrapper
          sx={{
            display: "flex",
